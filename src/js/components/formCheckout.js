...
function setLoading() {
  ...
  button.innerHTML = '<span uk-spinner></span>';
...
function showFieldErros(data) {
  [...element.querySelectorAll('input')].forEach(inputElement => inputElement.classList.remove('uk-form-danger'));
  let isFocused = false;
  console.log(data);
  Object.keys(data).forEach((key) => {
    ...
    if (inputElement) {
      ...
      errorElement.classList.add('error', 'uk-text-danger', 'uk-text-small');
...
function submit() {
  ...
  fetch(element.action, {
    ...
    if (data.status === 201) {
      ...
      errorElement.classList.add('uk-text-danger', 'uk-margin-bottom');
